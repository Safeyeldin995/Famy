-- QA-only helper: reset a registry provider back to DRAFT without deleting rows
-- referenced by immutable booking history. Limited to QA_ profiles.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE p.id = p_profile_id AND p.full_name ILIKE 'QA\_%' ESCAPE '\'
  ) THEN
  ...
  IF FOUND THEN
    ...
    UPDATE public.profiles SET
      full_name = COALESCE(full_name, 'QA_provider_e2e'),
      avatar_url = NULL
