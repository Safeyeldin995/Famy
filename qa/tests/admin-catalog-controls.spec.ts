...
async function assertClean(
  page: Page,
  readErrors: ReturnType<typeof captureErrors>["readErrors"],
) {
...
function cleanupError(label: string, error: unknown) {
  const message =
...
async function collectCleanupError(
  errors: Error[],
  label: string,
  task: () => Promise<{ error?: unknown } | void>,
) {
  try {
    const result = await task();
    if (result?.error) {
      errors.push(cleanupError(label, result.error));
    }
  } catch (error) {
...
function throwAfterCleanup(
  ...
  if (testError && cleanupErrors.length > 0) {
    ...
    throw new AggregateError(
      ...
      `${cleanupMessage} The test also failed.`,
    );
  }
...
async function createPaymentMethod(page: Page, code: string, name: string) {
  ...
  await Promise.all([
    page.waitForResponse(
      (response) =>
        response.url().includes("/rest/v1/payment_methods") &&
...
test("payment method edit, activation, default, and ordering persist", async ({
  page,
}, testInfo) => {
  test.slow();
  ...
  const { data: originalDefaults, error: defaultsError } = await supabaseAdmin
  ...
  expect(
    originalDefaults,
    "payment default state must be snapshotted before mutation",
  ).toBeTruthy();
  const restorationId = `payment-defaults-${suffix}`;
  registerRestoration({
    id: restorationId,
    type: "payment_defaults",
    rows: originalDefaults,
  });
  ...
  try {
    ...
    expect(storedA).toMatchObject({
      name_en: `${nameA} edited`,
      is_active: true,
    });
    expect(storedB.is_default).toBe(true);
    expect(storedB.display_order).toBeLessThan(storedA.display_order);
    const beforeFailedSwap = stored!.map(({ id, display_order }) => ({
      id,
      display_order,
    }));
    const { error: forcedFailure } = await authenticatedClient("admin").rpc(
    ...
    const { data: afterFailedSwap } = await supabaseAdmin
      .from("payment_methods")
      .select("id,display_order")
      .in("id", [storedA.id, storedB.id])
      .order("display_order");
    expect(afterFailedSwap).toEqual(beforeFailedSwap);
    const { error: customerDenied } = await authenticatedClient("customer").rpc(
      "admin_swap_payment_method_order",
      {
        p_first_id: storedA.id,
    ...
    const { data: afterDeniedSwap } = await supabaseAdmin
      .from("payment_methods")
      .select("id,display_order")
      .in("id", [storedA.id, storedB.id])
      .order("display_order");
    ...
    if (createdIds.length)
      await supabaseAdmin.from("payment_methods").delete().in("id", createdIds);
    else
      await supabaseAdmin
        .from("payment_methods")
        .delete()
        .in("code", [codeA, codeB]);
    const { data: restoredDefaults } = await supabaseAdmin
      .from("payment_methods")
      .select("id")
      .eq("is_default", true)
      .order("id");
    expect(restoredDefaults?.map((row) => row.id)).toEqual(
      originalDefaults!.filter((row) => row.is_default).map((row) => row.id),
    );
  }
});

test("service pricing, activation, and requirement controls persist", async ({
  page,
}) => {
  test.slow();
  const { readErrors } = captureErrors(page);
  ...
  try {
    ...
    const createServiceButton = createForm.getByRole("button", {
    ...
    const { data: persistedService, error: persistedServiceError } =
      await supabaseAdmin
        .from("services")
    ...
    await serviceCard
      .getByPlaceholder(/^arabic name$/i)
      .fill("QA_ requirement ar");
    ...
    const { data: service } = await supabaseAdmin
      ...
      .eq("id", serviceId!)
      .single();
    const { data: firstRequirement } = await supabaseAdmin
      .from("service_requirements")
      .select("*")
      .eq("service_id", service!.id)
      .eq("name_en", requirementName)
      .single();
    const { data: secondRequirement } = await supabaseAdmin
      .from("service_requirements")
      .insert({
    ...
    await serviceCard
      .locator('div[class*="border-coral/40"]')
      .getByRole("button", { name: /^deactivate$/i })
      .click();
    ...
    await page.waitForLoadState("networkidle");
    await page.reload();
    const { data: storedService } = await supabaseAdmin
      .from("services")
      .select("*")
      .eq("id", service!.id)
      .single();
    ...
    expect(storedRequirements![1]).toMatchObject({
      id: firstRequirement!.id,
      name_en: `${requirementName} edited`,
      is_active: false,
    });
    const requirementOrder = storedRequirements!.map(({ id, sort_order }) => ({
      id,
      sort_order,
    }));
    const { error: failedRequirementSwap } = await authenticatedClient(
      "admin",
    ).rpc("admin_swap_service_requirement_order", {
      p_first_id: firstRequirement!.id,
      p_second_id: "00000000-0000-0000-0000-000000000000",
    });
    expect(
      failedRequirementSwap,
      "a missing requirement target must fail atomically",
    ).toBeTruthy();
    const { data: unchangedRequirements } = await supabaseAdmin
      .from("service_requirements")
      .select("id,sort_order")
      .in("id", [firstRequirement!.id, secondRequirement!.id])
      .order("sort_order");
    expect(unchangedRequirements).toEqual(requirementOrder);
    const { count: serviceAudits } = await supabaseAdmin
      .from("audit_logs")
      .select("id", { head: true, count: "exact" })
      .eq("entity", "services")
    ...
    if (serviceId) {
      ...
      await collectCleanupError(cleanupErrors, "deactivate service", () =>
        supabaseAdmin
          .from("services")
          .update({ is_active: false })
          .eq("id", serviceId),
      );
      await collectCleanupError(cleanupErrors, "delete service", () =>
        supabaseAdmin.from("services").delete().eq("id", serviceId),
      );
    } else {
      await collectCleanupError(
        cleanupErrors,
        "deactivate service by slug",
        () =>
          supabaseAdmin
            .from("services")
            .update({ is_active: false })
      ...
      await collectCleanupError(cleanupErrors, "delete service by slug", () =>
        supabaseAdmin.from("services").delete().eq("slug", slug),
      );
    }
    ...
    throwAfterCleanup(
      testError,
      cleanupErrors,
      "Admin service fixture cleanup failed.",
    );
  }
});

test("promo edit and activation persist without a silent scope write", async ({
  page,
}) => {
  const { readErrors } = captureErrors(page);
  ...
  await page.getByLabel(/^code$/i).fill(code);
  await page
    .getByLabel(/discount \(egp\)|discount value|fixed amount/i)
    .first()
    .fill("15");
  ...
  const { count } = await supabaseAdmin
    .from("audit_logs")
    .select("id", { head: true, count: "exact" })
    .eq("entity", "promo_codes")
    .eq("entity_id", stored!.id);
  expect(count).toBeGreaterThanOrEqual(4);
  await assertClean(page, readErrors);
  await supabaseAdmin.from("promo_codes").delete().eq("id", stored!.id);
});

test("zone edit, activation, and service/provider coverage persist", async ({
  page,
}) => {
  test.slow();
  const { readErrors } = captureErrors(page);
  ...
  try {
    ...
    const providerUser = readRegistry().users.find(
      (user) => user.key === "provider",
    );
    if (!providerUser?.userId) {
      throw new Error(
        "Registry provider user missing. Run Playwright global setup first.",
      );
    }
    ...
    const { data: provider } = await supabaseAdmin
      .from("providers")
      .select("id,is_verified,is_active,onboarding_status")
    ...
    const { data: stored } = await supabaseAdmin
      .from("zones")
      .select("travel_fee,is_active")
      .eq("id", zone!.id)
      .single();
    expect(stored).toMatchObject({ travel_fee: 17, is_active: true });
    const { count: serviceCoverage } = await supabaseAdmin
      .from("zone_services")
      .select("zone_id", { count: "exact", head: true })
      .eq("zone_id", zone!.id)
      .eq("service_id", service!.id);
    const { count: providerCoverage } = await supabaseAdmin
      .from("zone_providers")
      .select("zone_id", { count: "exact", head: true })
      .eq("zone_id", zone!.id)
    ...
    const { count: audits } = await supabaseAdmin
      .from("audit_logs")
      .select("id", { count: "exact", head: true })
      .eq("entity", "zones")
      .eq("entity_id", zone!.id);
    ...
    if (zoneId) {
      await collectCleanupError(
        cleanupErrors,
        "delete zone service coverage",
        () =>
          supabaseAdmin.from("zone_services").delete().eq("zone_id", zoneId),
      );
      await collectCleanupError(
        cleanupErrors,
        "delete zone provider coverage",
        () =>
          supabaseAdmin.from("zone_providers").delete().eq("zone_id", zoneId),
      );
      await collectCleanupError(cleanupErrors, "deactivate zone", () =>
        supabaseAdmin
          .from("zones")
          .update({ is_active: false })
          .eq("id", zoneId),
      );
      await collectCleanupError(cleanupErrors, "delete zone", () =>
        supabaseAdmin.from("zones").delete().eq("id", zoneId),
      );
    }
    ...
    throwAfterCleanup(
      testError,
      cleanupErrors,
      "Admin zone fixture cleanup failed.",
    );
  }
});
