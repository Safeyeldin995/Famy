  ...
  return (
    <div className="space-y-4">
        ...
        {weekDays.map((day) => {
          ...
          return (
            <button
              ...
              type="button"
              disabled={!open}
              onClick={() => open && setSelectedWeekday(day.weekday)}
              className={`focus-ring relative flex min-w-[3.25rem] flex-1 flex-col items-center rounded-[1.25rem] px-2 py-3 transition-all ${
                selected
                  ? "bg-brand text-brand-foreground shadow-[0_10px_24px_-14px_var(--brand)]"
                  : open
                    ? "bg-brand/10 text-brand"
                    : "bg-surface-2 text-muted-foreground opacity-70"
              } ${!open ? "cursor-not-allowed" : "tap-scale"}`}
            >
              {isToday ? (
                <span
                  className={`absolute -top-1 rounded-full px-1.5 py-0.5 text-[9px] font-extrabold uppercase leading-none ${
                    selected ? "bg-white/20 text-white" : "bg-brand text-brand-foreground"
                  }`}
                >
        ...
        <div className="flex items-start gap-3">
          <div
            className={`grid h-11 w-11 shrink-0 place-items-center rounded-full ${
              selectedOpen ? "bg-brand text-brand-foreground" : "bg-border/60 text-muted-foreground"
            }`}
          >
            ...
            {selectedOpen ? (
              <div className="mt-1 space-y-1">
                {selectedRules.map((rule, index) => (
                  <p key={`${rule.weekday}-${index}`} className="text-base font-bold text-brand">
      ...
      <div className="space-y-2">
        {weekDays
          .filter((day) => rulesByWeekday.has(day.weekday))
          .map((day) => {
            ...
            return (
              <button
                ...
                className={`focus-ring flex w-full items-center justify-between gap-3 rounded-[1rem] px-3 py-2.5 text-start transition-colors ${
              ...
              >
                <span className="text-sm font-extrabold text-foreground">
                  ...
                  {isToday ? (
                    <span className="ms-2 text-[10px] font-bold uppercase text-brand">
                      {t("providerProfile.todayLabel")}
                    </span>
                  ) : null}
                </span>
