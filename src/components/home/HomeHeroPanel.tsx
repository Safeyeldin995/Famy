        ...
        <div className="-mx-5 mt-3 overflow-x-auto no-scrollbar">
          <div className="flex gap-2.5 px-5 pb-1">
            {categories.map((category) => (
              ...
              >
                <span className="grid h-12 w-12 place-items-center rounded-2xl bg-white/12 text-brand">
            ...
            {categoriesLoading
              ? Array.from({ length: 4 }).map((_, index) => (
                  <div key={index} className="h-[7.5rem] w-[5.75rem] shrink-0 animate-pulse rounded-2xl bg-white/10" />
                ))
              : null}
          </div>
        </div>
