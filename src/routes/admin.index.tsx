...
function AdminHome() {
  ...
  return (
    <div className="px-5 py-5 space-y-5">
      <div>
        <h1 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Overview</h1>
        {kpis.isLoading ? (
          ...
          <p className="mt-2 text-sm text-coral">Could not load KPIs. Please refresh.</p>
        ) : (
          <div className="mt-2 grid grid-cols-2 gap-3 lg:grid-cols-5">
            <KpiCard icon={Wallet} label="Revenue (captured)" value={formatEGP(kpis.data!.revenue)} tone="text-navy" />
            <KpiCard icon={ClipboardList} label="Active bookings" value={String(kpis.data!.activeBookings)} tone="text-coral" />
            <KpiCard icon={Clock} label="Pending bookings" value={String(kpis.data!.pendingBookings)} tone="text-amber-600" />
            <KpiCard icon={UserCheck} label="Active providers" value={String(kpis.data!.activeProviders)} tone="text-navy" />
            <KpiCard icon={Users} label="Active customers" value={String(kpis.data!.activeCustomers)} tone="text-coral" />
          </div>
        )}
      </div>
