...
import { EmptyState, PhoneFrame, StatusPill } from "@/components/famio/ui";
...
function AvailablePromoCard({ offer }: { offer: FeaturedPromoCode }) {
  ...
  return (
    <div className="home-ink-panel relative overflow-hidden rounded-[1.75rem] p-4 shadow-card">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-lg font-black tracking-tight">{discount}</p>
          {description ? <p className="mt-1 text-sm text-white/75">{description}</p> : null}
          {expiry ? <p className="mt-2 text-xs font-semibold text-white/60">{expiry}</p> : null}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-mono text-sm font-extrabold text-foreground" dir="ltr">
              {code}
            </span>
          ...
          <p className="mt-2 text-xs font-semibold text-muted-foreground">
            {formatDate(new Date(row.redeemed_at), {
              day: "numeric",
              month: "short",
              year: "numeric",
            })}
...
function PromoCodes() {
  ...
  return (
    <PhoneFrame bg="bg-background">
      <CustomerPageHero title={t("promoCodes.title")} backTo="/profile" />
      <div className="flex-1 space-y-8 px-5 pb-10 pt-2">
        <section>
          ...
          {availableQ.isLoading ? (
            ...
            <EmptyState
              icon="default"
              title={t("promoCodes.availableEmpty")}
              body={t("promoCodes.availableEmptyBody")}
            />
          ) : (
            <div className="space-y-3">
              {available.map((offer) => (
        ...
        <section>
          ...
          {historyQ.isLoading ? (
            ...
            <EmptyState
              icon="default"
              title={t("promoCodes.historyEmpty")}
              body={t("promoCodes.historyEmptyBody")}
            />
          ) : (
            <div className="space-y-3">
              {history.map((row) => (
