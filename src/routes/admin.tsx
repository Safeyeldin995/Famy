...
const NAV_GROUPS: (NavGroup & { titleFallback: string })[] = [
  {
    ...
    <div className="min-h-dvh bg-[#FEFAFC] text-foreground">
      ...
      <div className="mx-auto flex max-w-[90rem] gap-0 lg:gap-6 lg:px-6 lg:py-5">
        ...
        >
          <div className="flex items-center justify-end border-b border-border/40 p-2">
            <button
              ...
              aria-label={collapsed ? t("admin.layout.expandSidebar", "Expand sidebar") : t("admin.layout.collapseSidebar", "Collapse sidebar")}
            >
              {collapsed ? <PanelLeft className="h-4 w-4" /> : <PanelLeftClose className="h-4 w-4" />}
            </button>
          </div>
          <nav className="max-h-[calc(100dvh-8rem)] overflow-y-auto p-2" aria-label={t("admin.layout.navAria", "Admin navigation")}>
        ...
        <main className="min-w-0 flex-1 bg-surface lg:rounded-xl lg:border lg:border-border/50 lg:shadow-sm">
          <nav className="flex gap-1.5 overflow-x-auto border-b border-border/50 p-2 lg:hidden" aria-label={t("admin.layout.navAria", "Admin navigation")}>
            {flatTabs.map((tab) => {
              const active = isActive(tab);
              return (
                <Link
