...
function CampaignForm({ onCreated }: { onCreated: () => void }) {
  ...
  const handlePreview = () => {
    preview.mutate(target, { onError: (e: any) => toast.error(e?.message ?? "Could not load audience count") });
  };

  const handleCreate = () => {
    if (!valid) { toast.error("Fill in both languages for title and body."); return; }
    create.mutate(
      {
      ...
      },
      {
        ...
        onError: (e: any) => toast.error(e?.message ?? "Could not create campaign"),
      },
    );
  };
  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-surface p-5 shadow-card">
      <h2 className="text-sm font-extrabold">New campaign</h2>
      <p className="mt-0.5 text-xs text-muted-foreground">Drafts are created here; nothing sends until you activate it below.</p>

      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <input value={titleEn} onChange={(e) => setTitleEn(e.target.value)} placeholder="Title (English)"
          className="h-10 rounded-xl border border-border bg-surface px-3 text-sm" />
        <input value={titleAr} onChange={(e) => setTitleAr(e.target.value)} placeholder="العنوان (عربي)" dir="rtl"
          className="h-10 rounded-xl border border-border bg-surface px-3 text-sm" />
        <textarea value={bodyEn} onChange={(e) => setBodyEn(e.target.value)} rows={3} placeholder="Body (English)"
          className="resize-none rounded-xl border border-border bg-surface p-2 text-sm sm:col-span-1" />
        <textarea value={bodyAr} onChange={(e) => setBodyAr(e.target.value)} rows={3} dir="rtl" placeholder="النص (عربي)"
          className="resize-none rounded-xl border border-border bg-surface p-2 text-sm sm:col-span-1" />
      </div>

      <div className="mt-3 flex flex-wrap items-center gap-3">
        <select value={target} onChange={(e) => setTarget(e.target.value as CampaignTarget)}
          className="h-9 rounded-lg border border-border bg-surface px-2 text-xs font-semibold">
        ...
        <button onClick={handlePreview} disabled={preview.isPending}
          className="ml-auto rounded-lg border border-border px-3 py-1.5 text-xs font-bold disabled:opacity-50">
          {preview.isPending ? "Counting…" : "Preview audience"}
        </button>
      </div>
      {preview.data !== undefined && preview.isSuccess && (
        <p className="mt-2 text-xs text-muted-foreground">Estimated recipients: <span className="font-bold text-foreground">{preview.data}</span></p>
      )}

      <button onClick={handleCreate} disabled={create.isPending || !valid}
        className="mt-4 rounded-xl bg-navy px-4 py-2 text-xs font-bold text-navy-foreground disabled:opacity-50">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[10px] text-muted-foreground">
            target: {c.target} · push: {c.channel_push ? "yes" : "no"}
            {c.scheduled_for && ` · scheduled: ${new Date(c.scheduled_for).toLocaleString()}`}
            {c.status === "sent" && ` · delivered: ${deliveryQ.data ?? c.recipient_count ?? "…"}`}
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          {c.status === "draft" && (
            <button
              onClick={() => activate.mutate(c.id, { onError: (e: any) => toast.error(e?.message ?? "Could not activate campaign") })}
              disabled={activate.isPending}
              className="inline-flex items-center gap-1 rounded-lg bg-navy px-3 py-1.5 text-[11px] font-bold text-navy-foreground disabled:opacity-50"
            >
              <Send className="h-3 w-3" /> Activate
            </button>
          )}
          {(c.status === "draft" || c.status === "scheduled") && (
            <button
              onClick={() => cancel.mutate(c.id, { onError: (e: any) => toast.error(e?.message ?? "Could not cancel campaign") })}
              disabled={cancel.isPending}
              className="inline-flex items-center gap-1 rounded-lg border border-coral px-3 py-1.5 text-[11px] font-bold text-coral disabled:opacity-50"
            >
  ...
  return (
    <div className="space-y-4 px-5 py-5">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Notification Campaigns</h1>
      </div>
      ...
      <div className="rounded-2xl border border-border/60 bg-surface p-5 shadow-card">
        ...
        {q.isLoading ? (
          ...
          <p className="mt-3 text-sm text-coral">Could not load campaigns. Please refresh.</p>
        ) : (q.data ?? []).length === 0 ? (
