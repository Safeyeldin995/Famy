...
Verified 2026-07-16 against local runtime and Vercel Preview deployment
`dpl_2fjfk82ZDfoDNC8N1FFtbPUj7KRj`. Both gates passed 36/36 with the real
...
|---|---|---|---|---|---|---|---|---|
| Overview | Read | KPIs and queues load at `/admin` with loading/error/retry states | payments, bookings, providers, profiles | `admin-reads.spec.ts` | PASS | PASS | N/A | PASS |
...
All read pages render a loading state, an empty state, and a useful error with
an explicit retry action. No tested route produced an endless spinner, blank
screen, unexpected console error, or unexpected failed request.
...
| Activate/deactivate/reorder requirement | Write | Serialized reorder and verified state | service_requirements | `admin-catalog-controls.spec.ts` | PASS | PASS | Refresh/audit/cleanup PASS | PASS |
...
| Reorder Payment Methods | Write | Serialized two-row swap persists | payment_methods | `admin-catalog-controls.spec.ts` | PASS | PASS | Refresh/audit/cleanup PASS | PASS |
...
| Reorder Cancellation Reasons | Write | Serialized two-row swap supported | cancellation_reasons | query/runtime coverage | PASS | PASS | Verified readback | PASS |
...
| Update Settings | Write | Billing, categories, areas, reminders and content persist | settings, categories, booking_reminder_rules | `admin-writes.spec.ts` | PASS | PASS | Refresh/audit/cleanup PASS | PASS |
...
- Two-row reorder actions issued concurrent updates. They are now serialized and
...
- Vercel Preview had stale Supabase credentials and missing server variables.
  All nine Preview values were refreshed from the verified environment.
- Preview SSO testing now uses a temporary Vercel bypass cookie stored only in QA
  browser state; the automation token was removed after the run.

No additive migration was required. Existing RLS, Admin checks, mandatory
requirement guards, immutable audit history, and polygon-zone implementation
remain intact.
