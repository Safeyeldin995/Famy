...
| Production deployment readiness | **Runbook landed; one investigation open** | `docs/PRODUCTION_DEPLOYMENT_RUNBOOK.md`. PITR/backups confirmed enabled on Production (2026-08-24). Migration-apply process: a manual Supabase SQL Editor process was confirmed working end-to-end on 2026-08-27 (all 3 pending migrations now applied to Production, including monitoring/error-logs and rate limiting from PR #48). Open: 2 of those 3 migrations were found already present on Production *before* being manually applied — cause not yet identified (possible undocumented auto-apply integration); see the runbook's Migration application order section for the exact finding and recommended check. |
