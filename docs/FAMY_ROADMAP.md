...
| Production deployment readiness | **DRAFT runbook exists (2026-08-24), unreviewed** | `docs/PRODUCTION_DEPLOYMENT_RUNBOOK.md` — env var checklist compiled from real `process.env`/`import.meta.env`/`Deno.env` usage (not the stale `.env.example`), migration/rollback sections. Three open questions block calling it final: (1) what's the actual Production migration-apply process — never documented anywhere; (2) is Supabase PITR/backup enabled on Production, and what's the retention window — determines whether a real rollback mechanism exists at all, since this repo has zero down-migrations or rollback tooling; (3) monitoring/alerting (errors, failed payments, failed notifications) is still genuinely not wired up — that's separate implementation work, not a docs fix. Needs Safeyeldin's review and answers before Milestone 3's runbook exit criterion is met. |
